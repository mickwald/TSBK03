...
void runfilter(GLuint shader, FBOstruct *out, FBOstruct *in1, FBOstruct *in2, int loops)
{
	loops--;
	//printf("%d \n", loops);
	if(loops>=0){
	    	...
	    	DrawModel(squareModel, shader, "in_Position", NULL, NULL); //"in_TexCoord"
	    	glFlush();
	
		runfilter(shader, in1, out, in2, loops);
	}
}
