...
void DeformCylinder()
{
  ...
  //float w[kMaxBones];
  int row, corner, i, j;
	mat4 invBonesPos[kMaxBones];
	mat4 bonesMat[kMaxBones];
	mat4 bonesComplete[kMaxBones];
	Point3D res = SetVector(0.0,0.0,0.0);
	
	for(j = 0; j < kMaxBones; j++){
		bonesComplete[j] = IdentityMatrix();
	}

	invBonesPos[0] = InvertMat4(T(g_bones[0].pos.x,g_bones[0].pos.y,g_bones[0].pos.z));
	bonesMat[0] = Mult(T(g_bones[0].pos.x,g_bones[0].pos.y,g_bones[0].pos.z),g_bonesRes[0].rot);
	bonesComplete[0] = Mult(bonesMat[0], invBonesPos[0]);

	//for each bone
	
	for(i = 1; i < kMaxBones; i++)
	{
		invBonesPos[i] = InvertMat4(T(g_bones[i].pos.x - g_bones[i-1].pos.x,
		g_bones[i].pos.y - g_bones[i-1].pos.y,
		g_bones[i].pos.z - g_bones[i-1].pos.z));

		bonesMat[i] = Mult(T(g_bones[i].pos.x - g_bones[i-1].pos.x,
		g_bones[i].pos.y - g_bones[i-1].pos.y,
		g_bones[i].pos.z - g_bones[i-1].pos.z),g_bonesRes[i].rot);

		for(j = i; j>=0; j--){
			bonesComplete[i] = Mult(bonesMat[j],Mult(bonesComplete[i],invBonesPos[j]));
		}
  ...
  for (row = 0; row < kMaxRow; row++)
  {
	...
	g_vertsRes[row][corner] = res;
	res = SetVector(0.0,0.0,0.0);
...
void animateBones(void)
{
	...
	//float angleScales[10] = { -1.f, 1.f, -1.f, 1.f, -1.f, 1.f, -1.f, 1.f, -1.f, 1.f };
	//float angleScales[10] = { 0.1f, 0.2f, 0.3f, 0.4f, 0.5f, 0.6f, 0.7f, 0.8f, 0.9f, 1.0f };
	//float angleScales[10] = { 0.1f, 0.2f, 0.3f, 0.4f, 1.5f, 0.6f, 0.7f, 0.8f, 0.9f, 1.0f };
	//float angleScales[10] = { 100.f, 10.f, 0.f, 5.f, 0.f, 10.f, 0.f, 5.f, 0.f, 10.0f };
